...
public class Matrix
{
    ...
    {
        this.matrix = matrix;
        double matrixSize = Math.Sqrt(matrix.Length);
        this.size = (int)matrixSize;
    }
    ...
    public void AddLine(string[] lineTokens)
    {
        for (int i = 0; i < this.size; i++)
        {
            ...
            {
                Console.WriteLine(lineTokens[i] + " at position " + i +
                                  " is not a valid number, please enter a valid number into console:");
                string newNum = Console.ReadLine();
                while (!int.TryParse(newNum, out value))
                {
                    Console.WriteLine("number is not valid, please try again");
    ...
    public void matrixPrint()
    {
        for (int i = 0; i < rowNumber; i++)
        {
            ...
            {
                if (matrix[i, j].ToString().ToCharArray().Length == 4)
                {
                    line += matrix[i, j] + " ";
                }
                else if (this.matrix[i, j].ToString().ToCharArray().Length == 3)
                {
                    line += matrix[i, j] + "  ";
                }
